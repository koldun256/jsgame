			...
			if (
				...
				if (collider.isTouching(listener.collider)) {
					if (listener.eventType == "enter") {
						if (!listener.currentlyEntered.has(collider.id))
							listener.callback(collider.owner);
					}
					if (listener.eventType == "stay")
						listener.callback(collider.owner);
					listener.currentlyEntered.add(collider.id);
				} else {
					if (
						listener.eventType == "exit" &&
						listener.currentlyEntered.has(collider.id)
					)
						listener.callback(collider.owner);
